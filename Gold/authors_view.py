...
def processData(data):
    ...
    data = data.groupBy("author").agg(f.avg("average_rating").alias("average_rating"),\
    ...
    data = data.where(f.col("author").isNotNull())
    return data

def saveData(df):
    df.write.mode("overwrite").parquet(awsS3Directory + "/Gold/authorsView")
