...
def processData(data):
    ...
    data = data.groupBy("language").agg(f.avg("average_rating").alias("average_rating"),\
    ...
    data = data.where(f.col("language").isNotNull())
    return data

def saveData(df):
    df.write.mode("overwrite").parquet(awsS3Directory + "/Gold/languagesView")
